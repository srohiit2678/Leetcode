...
Output: false
Explanation:
The strings s and t can not be made identical as 'o' needs to be 
...
public class LeetCode205 {
  static boolean isIsomorphic(String s, String t) {
   HashMap<Character,Character> contain = new HashMap<Character,Character>();
  for(int i=0;i<s.length();i++){
    if(contain.containsKey(s.charAt(i))){
      if(!(contain.get(s.charAt(i)).equals(t.charAt(i)))){
        return false;
      }
    }
    else{
        if(contain.containsValue(t.charAt(i))){
          return false;
        }
      contain.put(s.charAt(i),t.charAt(i));
    }
  }
    return true;     
  }
